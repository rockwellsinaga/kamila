...
dateInput.addEventListener('change', function() { updateUI(this.value); });
...
checkStatusBtn.addEventListener('click', function() { checkRecommendation(dateInput.value); });
...
function updateUI(currentDate) {
    ...
    if (!hasLogs) {
        sportList.innerHTML = '<li class="list-group-item empty-state">Belum ada olahraga hari ini. Ayo mulai bergerak!</li>';
    }
...
function checkRecommendation(currentDate) {
    ...
    if (totalDurationToday === 0) {
        alert("Peringatan: Anda belum olahraga hari ini!\n\nMenurut WHO, disarankan berolahraga minimal 30 menit per hari.");
    } else if (totalDurationToday < MIN_DURATION_RECOMMENDED) {
        alert(`Anda baru berolahraga ${totalDurationToday} menit. Kurang ${MIN_DURATION_RECOMMENDED - totalDurationToday} menit lagi dari standar WHO!`);
    } else {
